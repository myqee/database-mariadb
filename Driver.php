...
class Driver extends DriverSQL
{
    ...
    public function closeConnect()
    {
        if ($this->connections)
        {
            foreach ($this->connections as $key => $connection)
            {
                ...
                if(true === INCLUDE_MYQEE_CORE && IS_DEBUG)Core::debug()->info('close '. $key .' connection.');
            }
    ...
    public function query($sql, $asObject = null, $clusterName = null)
    {
        ...
        # 记录调试
        if(true === INCLUDE_MYQEE_CORE && IS_DEBUG)
        {
        ...
        if (null === $isNoCache) $isNoCache = (true === INCLUDE_MYQEE_CORE && IS_DEBUG) ? (bool)Core::debug()->profiler('nocached')->is_open() : false;
        ...
        if (isset($benchmark))
        {
            ...
            if(true === INCLUDE_MYQEE_CORE && IS_DEBUG)Core::debug()->profiler('sql')->stop($data);
        }
